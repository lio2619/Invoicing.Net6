...
{
    public partial class IncomingOutgoingOrderForm : Form
    {
        ...
        private int _end = 40;
        private int _page = 1;
        public IncomingOutgoingOrderForm()
        {
            InitializeComponent();
        ...
        private void productDetailData_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                commonFormFunction.ReadItemDetailIncomingOutgoingOrder(productDetailData, e.RowIndex, itemNumber, customerName.Text);
        ...
        {
            printFunction.PrintPage(productDetailData, e, dateTime, customerName.Text, orderNumber.Text, "進貨退出單", remarkTextBox.Text, totalMoney.Text, _page, _start, _end);
            _page++;
            _start += 40;
            _end += 40;
        ...
        private void saveOrderButton_Click(object sender, EventArgs e)
        {
            ...
            CommonButtonFunction commonButtonFunction = new CommonButtonFunction(productDetailData, dateOnly, storeName, remark, totalPrice, "進貨退出單");
            if (string.IsNullOrEmpty(orderNumber.Text))
            {
        ...
        {
            SearchOrderForm searchOrderForm = new SearchOrderForm("進貨退出單", productDetailData, orderNumber, totalMoney, remarkTextBox, customerName, dateTime);
        ...
        private void createExcelButton_Click(object sender, EventArgs e)
        {
            ...
            string path = "../單子/" + dateMonth + "/進貨退出單/";
            string fileName = "進貨退出單_" + dateTime.Value.ToString("yyyy-MM-dd") + "_" + customerName.Text.ToString();
            CreateExcelFunction createExcelFunction = new CreateExcelFunction(productDetailData, dateMonth, dateMonthDay, fileName, orderNumber.Text, customerName.Text, "進貨退出單");
