...
namespace invoicing.Forms
{
    public partial class OrderForm : Form
    {
        ...
        {
            printFunction.PrintPage(productDetailData, e, dateTime, customerName.Text, orderNumber.Text, "訂貨單", remarkTextBox.Text, totalMoney.Text, _page, _start, _end);
            _page++;
            _start += 40;
            _end += 40;
        ...
        private void saveOrderButton_Click(object sender, EventArgs e)
        {
            ...
            CommonButtonFunction commonButtonFunction = new CommonButtonFunction(productDetailData, dateOnly, storeName, remark, totalPrice, "訂貨單");
            if (string.IsNullOrEmpty(orderNumber.Text))
            {
        ...
        {
            SearchOrderForm searchOrderForm = new SearchOrderForm("訂貨單", productDetailData, orderNumber, totalMoney, remarkTextBox, customerName, dateTime);
        ...
        private void createExcelButton_Click(object sender, EventArgs e)
        {
            ...
            string path = "../單子/" + dateMonth + "/訂貨單/";
            string fileName = "訂貨單_" + dateTime.Value.ToString("yyyy-MM-dd") + "_" + customerName.Text.ToString();
            CreateExcelFunction createExcelFunction = new CreateExcelFunction(productDetailData, dateMonth, dateMonthDay, fileName, orderNumber.Text, customerName.Text, "訂貨單");
